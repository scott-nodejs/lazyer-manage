      ...
      <section class="order_detail">
        <nav class="nav_container">
          <div class="box">
            ...
            <span class="order_name">订单号：{{order_id}}</span>
...
<!--            <el-button @click="printPdf" class="pdf fr">导出订单</el-button>-->
        ...
        <div class="content_box">
          ...
          <p>姓名: {{orderDetail.addressVo.name}}</p>
          <p>电话: {{orderDetail.addressVo.phone}}</p>
          <p>地址: {{orderDetail.addressVo.address}}</p>
          <p></p>
          <p>兑换物品: {{orderDetail.goodsVo.goods.goodsTitle}}</p>
          <p>状态: {{orderDetail.order.status == 2 ? '待发货' : orderDetail.order.status == 3 ? "已发货" : "已签收"}}</p>
    ...
    methods: {
      _fetchOrderDetail() {
        ...
        this.$http.get('order/detail', {params}).then(res => {
          ...
          //this.$store.commit(UPDATE_ORDER_DETAIL, res.data)
