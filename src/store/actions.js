...
export const fetchGoodsList = function({state, commit}, params) {
  ...
  Axios.get('goods/list', {params: JSON.parse(newParamsStr)}).then(res => {
    if(!res) return
    if(!res.data) {
...
export const fetchActivityList = function({state, commit}, params) {
  commit(types.CHANGE_FETCH_GOODS_LIST_PARAMS, params)
  // current_id 三个参数只是记录商品菜单的层级，请求 API 时不需要这三个参数
  // 列举需要的参数
  const paramsList = [
  ...
  Axios.get('activity/list', {params: JSON.parse(newParamsStr)}).then(res => {
    if(!res) return
    if(!res.data) {
      commit(types.UPDATE_ACTIVITY_LIST, {})
    } else {
      commit(types.UPDATE_ACTIVITY_LIST, res.data)
    }
  })
}
...
export const fetchGoodsCategoryList = function({state, commit}){
  if(!state.goodsCategoryList.length) {
    Axios.get('goods/goods/category').then(res => {
